# Import party
import sqlite3, markdown, time, re
...
def view_post(id):
  ...
  clnPost = re.compile('<.*?>')
  strPal = re.sub(clnPost, '', posts[0]["text"])

  valPal = is_palindrome(strPal)
  if valPal:
      resPal = "Message :"+strPal+": is a palindrome"
  else:
      resPal = "Message :"+strPal+": not palindrome"
  return jsonify({
      'respose': posts[0],
      'Palindrome Results': resPal
    })
...
def add_post():
  ...
  return jsonify({
        'status': 'Added Post: '+request.form['title']+':'+ request.form['text']
		})
...
def update_post(postid):
  ...
  #return redirect(url_for('home'))
  return jsonify({
        'status': 'Updated: '+request.form['title']+':'+ request.form['text']
		})
...
@app.route('/del/<int:id>' , methods=['POST', 'DELETE'])
def delete_post(id):
  #if not session.get('logged_in'):
  #  abort(401)
...
def edit_post(id):
  ...
  #return render_template('delete.html', post=posts[0], id=id)
